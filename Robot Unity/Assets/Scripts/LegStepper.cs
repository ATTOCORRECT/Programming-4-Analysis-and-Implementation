...
public class LegStepper : MonoBehaviour
{
    ...
    IEnumerator Move()
    {
        ...
        Vector3 overshootVector = towardHome * stepOvershootFraction;
        ...
        // Apply the overshoot
        Vector3 endPoint = overshootVector;

        // We want to pass through the center point
        Vector3 centerPoint = (startPoint + endPoint + homeTransform.position) / 2;
        // But also lift off, so we move it up by half the step distance (arbitrarily)
        centerPoint += homeTransform.up * Vector3.Distance(startPoint, endPoint + homeTransform.position) / 2f;
        ...
        do
        {
            ...
            transform.position =
              Vector3.Lerp(
                ...
                Vector3.Lerp(centerPoint, endPoint + homeTransform.position, normalizedTime),
                normalizedTime
              );
